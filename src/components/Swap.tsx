...
import type { SwapToken, QuoteHop, QuoteTx, QuoteResponse, ArbResponse } from '../types';
...
export const Swap = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      axios.get<{ tokens: Array<{ identifier: string; ticker: string; poolCount: number; decimals: number; logoUrl?: string | null }> }>(`${apiUrl}/tokens`),
      axios.get(`${apiUrl}/tokens/hub`).catch(() => ({ data: { hubTokens: [] } })),
    ])
  ...
  }, [tokenIn, tokenOut, amountIn, isArb, isWrapUnwrap, slippage]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const amountOutDisplay = isWrapUnwrap
    ? amountIn
      ? new BigNumber(amountIn).toFixed(6, BigNumber.ROUND_DOWN)
      : ''
    : isArb && arb
          ...
          <div className="rounded-2xl border border-gray-200 dark:border-[#333] bg-gray-50 dark:bg-[#1e1e1e] p-4">
            ...
            <div className="flex items-center gap-3">
              <TokenSelect
                ...
                loading={tokensLoading}
              />
              <div className="w-28 xs:w-36 flex-shrink-0 rounded-xl border border-gray-200 dark:border-[#444] bg-white/60 dark:bg-[#2a2a2a]/60 px-3 py-2.5 text-right text-sm font-semibold text-gray-900 dark:text-white select-none overflow-hidden">
                {amountOutDisplay ? (
                  <span className="block truncate" title={amountOutDisplay}>
                    {amountOutDisplay}
                  </span>
                ) : (
