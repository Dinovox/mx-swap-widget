...
export const AddLiquidity = () => {
  ...
  const selectTokenA = (t: DexToken | null) => {
    setTokenA(t);
    setSearchParams(
      (prev) => {
        const p = new URLSearchParams(prev);
        t ? p.set("tokenA", t.identifier) : p.delete("tokenA");
        return p;
      },
      { replace: true },
    );
  };
  const selectTokenB = (t: DexToken | null) => {
    setTokenB(t);
    setSearchParams(
      (prev) => {
        const p = new URLSearchParams(prev);
        t ? p.set("tokenB", t.identifier) : p.delete("tokenB");
        return p;
      },
      { replace: true },
    );
  ...
  useEffect(() => {
    ...
    if (qA && !tokenA)
      setTokenA(tokens.find((t) => t.identifier === qA) || null);
    if (qB && !tokenB)
      setTokenB(tokens.find((t) => t.identifier === qB) || null);
            ...
            <div className="rounded-2xl border border-gray-200 dark:border-[#333] bg-[#ffffff] dark:bg-[#1a1a1a] p-4 mt-4 space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-bold text-amber-500">
                  {new BigNumber(lpPreview.toString())
                    .shiftedBy(-18)
                    .toFixed(6)}{" "}
                  LP
                </span>
              ...
              {refundA > 0n && (
                <div className="flex justify-between text-xs">
                  ...
                  <span className="font-medium text-gray-700 dark:text-gray-300">
                    {new BigNumber(refundA.toString())
                      .shiftedBy(-(tokenA?.decimals ?? 18))
                      .toFixed(6)}
                  </span>
                </div>
              ...
              {refundB > 0n && (
                <div className="flex justify-between text-xs">
                  ...
                  <span className="font-medium text-gray-700 dark:text-gray-300">
                    {new BigNumber(refundB.toString())
                      .shiftedBy(-(tokenB?.decimals ?? 18))
                      .toFixed(6)}
                  </span>
                </div>
            ...
            disabled={
              !address
                ...
                : !pool ||
                  ...
                  !amountA ||
                  !amountB ||
                  (lpPreview !== null && lpPreview < 1000n && !poolHasLiquidity)
            }
