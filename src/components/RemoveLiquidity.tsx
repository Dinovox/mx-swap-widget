...
export const RemoveLiquidity = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (
          qPool &&
          activePools.some((p: LiquidityPool) => p.address === qPool)
        )
  ...
  const lpTokenBalances = useGetUserESDT(selectedPool?.lpToken ?? undefined, {
  ...
  const lpAmountRaw = new BigNumber(lpBalanceRaw)
    .multipliedBy(percentage)
    .dividedBy(100)
    .toFixed(0, BigNumber.ROUND_DOWN);
  const lpAmountDisplay = new BigNumber(lpAmountRaw)
    .shiftedBy(-18)
    .toFixed(6, BigNumber.ROUND_DOWN);
  const lpBalanceDisplay = new BigNumber(lpBalanceRaw)
    .shiftedBy(-18)
    .toFixed(6, BigNumber.ROUND_DOWN);
  ...
  const outADisplay = new BigNumber(outA)
    .shiftedBy(-decimalsA)
    .toFixed(6, BigNumber.ROUND_DOWN);
  const outBDisplay = new BigNumber(outB)
    .shiftedBy(-decimalsB)
    .toFixed(6, BigNumber.ROUND_DOWN);

  const tickerA =
    tokens[selectedPool?.tokenA ?? ""]?.ticker ??
    selectedPool?.tokenA?.split("-")[0] ??
    "";
  const tickerB =
    tokens[selectedPool?.tokenB ?? ""]?.ticker ??
    selectedPool?.tokenB?.split("-")[0] ??
    "";
  const priceA = tokens[selectedPool?.tokenA ?? ""]?.priceUsd;
  const priceB = tokens[selectedPool?.tokenB ?? ""]?.priceUsd;
  const outAUsd =
    priceA && new BigNumber(outA).gt(0) ? formatUsd(priceA, outADisplay) : null;
  const outBUsd =
    priceB && new BigNumber(outB).gt(0) ? formatUsd(priceB, outBDisplay) : null;
  ...
  const handleTx = async () => {
    ...
    try {
      ...
      const transaction = new Transaction({
        ...
        receiver: new Address(selectedPool.address),
      ...
      >
        <div className="space-y-4 mt-4">
          ...
          <div className="rounded-2xl border border-gray-200 dark:border-[#333] bg-gray-50 dark:bg-[#1e1e1e] p-4">
            ...
            >
              <option value="">
                {poolsLoading ? "Chargement..." : "Choisir une pool"}
              </option>
              {pools.map((p) => (
                <option key={p.address} value={p.address}>
                  {tokens[p.tokenA]?.ticker || p.tokenA} -{" "}
                  {tokens[p.tokenB]?.ticker || p.tokenB}
                </option>
              ))}
            </select>
          </div>
